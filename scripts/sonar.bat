...
REM Token: passed as 2nd arg, set SONAR_TOKEN env var, or loaded from .env.secrets.
...
REM Fallback: load token from .env.secrets if still empty
if "%TOKEN%"=="" (
...
if "%CMD%"=="up" (
    ...
    echo Running bootstrap (password + token)...
    call scripts\sonar-pw.bat
    REM Reload token after bootstrap may have written it to .env.secrets
    if "%TOKEN%"=="" (
        if exist ".env.secrets" (
            for /f "usebackq tokens=1,* delims==" %%A in (".env.secrets") do (
                if "%%A"=="SONAR_TOKEN" if not "%%B"=="" set "TOKEN=%%B"
