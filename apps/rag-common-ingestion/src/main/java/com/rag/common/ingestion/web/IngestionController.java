...
/**
 * REST endpoints for ingesting documents (raw content, multipart file or via
 * rag-webcrawler). Shared across rag-basic/rag-advanced/rag-agentic; the
 * page-fetching and document-management seams are injected per module.
 */
@RestController
...
public class IngestionController {
    ...
    public IngestionController(IngestionService ingestionService, WebPageFetcher webPageFetcher) {
        this(ingestionService, webPageFetcher, null, null);
    }

    @Autowired
    public IngestionController(IngestionService ingestionService, WebPageFetcher webPageFetcher,
                               AsyncIngestionService asyncIngestionService, DocumentStorePort documentStore) {
    ...
    public ResponseEntity<List<DocumentSummaryDTO>> listDocuments() {
        ...
        var documents = documentStore.listDocuments().stream()
    ...
    public ResponseEntity<IngestResponse> ingestUrl(@RequestBody IngestUrlRequest request) {
        ...
        var page = webPageFetcher.fetch(request.getUrl().toString());
