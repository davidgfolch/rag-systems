...
class ChatWebSocketHandlerTest {
    ...
    private final ChatWebSocketHandler handler = new ChatWebSocketHandler(answerStreamer, new ObjectMapper());
    ...
    void streamsTokensThenDone() throws Exception {
        when(answerStreamer.askStream("hello", 5))
        ...
        verify(answerStreamer).askStream("hello", 5);
    }

    @Test
    void cancelDisposesStream() throws Exception {
        when(answerStreamer.askStream("hello", 5)).thenReturn(Flux.never());
        send("{\"type\":\"ask\",\"question\":\"hello\",\"conversationId\":\"c1\"}");
    ...
    void ignoresEmptyQuestion() throws Exception {
        ...
        verify(answerStreamer, never()).askStream(anyString(), anyInt());
    }
    ...
    void usesDefaultTopKWhenNull() throws Exception {
        when(answerStreamer.askStream("hello", 5)).thenReturn(Flux.empty());
        send("{\"type\":\"ask\",\"question\":\"hello\",\"conversationId\":\"c1\"}");
        sentPayloads(1);
        verify(answerStreamer).askStream("hello", 5);
    }
    ...
    void usesExplicitTopKWhenProvided() throws Exception {
        when(answerStreamer.askStream("hello", 7)).thenReturn(Flux.empty());
        send("{\"type\":\"ask\",\"question\":\"hello\",\"topK\":7,\"conversationId\":\"c1\"}");
        sentPayloads(1);
        verify(answerStreamer).askStream("hello", 7);
    }

    @Test
    void runsWithoutTracerSpanInSessionAttributes() throws Exception {
        when(answerStreamer.askStream("hello", 5)).thenReturn(Flux.just("tok"));
        send("{\"type\":\"ask\",\"question\":\"hello\",\"conversationId\":\"c1\"}");
    ...
    void sendsSingleDoneWhenStreamCancelledBeforeCompletion() throws Exception {
        ...
        send("{\"type\":\"ask\",\"question\":\"hello\",\"conversationId\":\"c1\"}");
        handler.handleTextMessage(session, new TextMessage("{\"type\":\"cancel\",\"conversationId\":\"c1\"}"));
        verify(answerStreamer, atLeastOnce()).askStream(anyString(), anyInt());
    }
