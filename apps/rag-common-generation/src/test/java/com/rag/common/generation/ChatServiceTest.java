...
class ChatServiceTest {
    ...
    private final StreamingChatModelPort streamingChatModel = mock(StreamingChatModelPort.class);
    private final ChatService sut = new ChatService(vectorStore, chatModel, streamingChatModel);
    ...
    void streamsTokensFromChatModel() {
        ...
        when(streamingChatModel.completeStream(anyString()))
                .thenReturn(Flux.just("Spring", " AI", " works."));
        var stream = sut.askStream("q", 2);
        StepVerifier.create(stream)
                .expectNext("Spring", " AI", " works.")
                .verifyComplete();
        verify(streamingChatModel).completeStream(anyString());
    }
    ...
    void streamsWithNoContextPlaceholder() {
        when(vectorStore.similaritySearch("q", 2)).thenReturn(List.of());
        when(streamingChatModel.completeStream(anyString())).thenReturn(Flux.just("I don't know."));
        StepVerifier.create(sut.askStream("q", 2)).expectNext("I don't know.").verifyComplete();
        verify(streamingChatModel).completeStream(contains("(no context retrieved)"));
    }
}