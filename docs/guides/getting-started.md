...
Configuration lives in the committed `.env` at the repo root (profiles, model
defaults, ports, module registry). Secrets (API keys, passwords, tokens) live in
`.env.secrets`, generated by `scripts/bootstrap-env.{bat,sh}` from
`scripts/.env.secrets.example` — copied to the repo root **only if it doesn't
already exist**, with random `PGVECTOR_PASSWORD` and `SONAR_ADMIN_PASSWORD`
generated for blank values. `.env.secrets` is gitignored.

No manual copy is needed — just run any script, then edit `.env` to override
defaults (e.g. profile), and edit `.env.secrets` for any secrets
...
The API starts on http://localhost:8081 with health at http://localhost:8081/actuator/health. The API contract is defined once in [rag-api.yaml](../../apps/rag-contract/src/main/resources/openapi/rag-api.yaml) (there is no per-module Swagger UI). See the [rag-basic README](../../apps/rag-basic/README.md) for endpoints, quick-access URLs, and example `curl` calls.
