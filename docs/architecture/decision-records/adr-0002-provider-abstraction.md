...
Use **interfaces** (`EmbeddingModelPort`, `ChatModelPort`) defined in `rag-common-core`, with provider implementations selected via **Spring configuration profiles** (`local` for Ollama, `cloud` for OpenAI). No provider-specific code in business services.
